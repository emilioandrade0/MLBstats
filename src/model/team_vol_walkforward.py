"""Walk-forward: do team volatility (std L10) features help?

Tests whether std-of-runs features add signal over existing means.
"""
...
def main():
    ...
    tv_feats = [c for c in df.columns if any(
        s in c for s in ("runs_scored_std_l10", "runs_allowed_std_l10", "run_diff_std_l10")
    )]
    print(f"team_volatility feats in train: {len(tv_feats)}")
    ...
    base = [c for c in all_feats if c not in tv_feats]
    full = base + tv_feats
    print(f"baseline (no vol): {len(base)}  / +vol: {len(full)}")
    print()
    cat = ["home_team_id", "away_team_id"]
    print("=" * 90)
    a_b, u_b, l_b = _wf(df, base, [c for c in cat if c in base], "BASELINE")
    a_f, u_f, l_f = _wf(df, full, [c for c in cat if c in full], "+ team_vol")
    print()
    print("=" * 90)
