"""Walk-forward: does starter_l5 (last 5 starts) help on top of L15?

Baseline = current model (already includes lineup_recent + team_id + L15 starter)
+L5 = same + starter_xwoba_l5, starter_k_pct_l5, etc. (7 base × 3 = 21 cols)

Run:
  python -m src.model.starter_l5_walkforward
"""
...
def main():
    ...
    l5_feats = [c for c in df.columns if "_l5" in c and c.startswith("starter_")]
    print(f"starter_l5 feats in train: {len(l5_feats)}")
    # All non-L5 numeric features
    ...
    base = [c for c in all_feats if c not in l5_feats]
    full = base + l5_feats
    print(f"baseline (no L5): {len(base)}  / +L5: {len(full)}")
    print()
    cat = ["home_team_id", "away_team_id"]
    print("=" * 90)
    a_b, u_b, l_b = _wf(df, base, [c for c in cat if c in base], "BASELINE")
    a_f, u_f, l_f = _wf(df, full, [c for c in cat if c in full], "+ starter_l5")
    print()
    print("=" * 90)
    print(f"Δ acc:    {(a_f-a_b)*100:+.3f}pp")
    print(f"Δ AUC:    {(u_f-u_b)*1000:+.2f} (x1000)")
    print(f"Δ logloss:{(l_f-l_b):+.5f}  (negative = better calibration)")
