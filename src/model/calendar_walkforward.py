"""Walk-forward: do calendar features (slate_size + dow + is_small_slate) help?

Baseline = current production model (with lineup_recent + team_id, no calendar)
+cal = same + slate_size + dow (categorical) + is_small_slate

Run:
  python -m src.model.calendar_walkforward
"""
...
def main():
    ...
    cal_feats = [c for c in df.columns if c in ("slate_size", "dow", "is_small_slate")]
    print(f"calendar feats in train: {cal_feats}")
    ...
    base = [c for c in all_feats if c not in cal_feats]
    full = base + cal_feats
    print(f"baseline (no cal): {len(base)}  / +cal: {len(full)}")
    print()
    cat_base = [c for c in ("home_team_id", "away_team_id") if c in base]
    cat_full = [c for c in ("home_team_id", "away_team_id", "dow") if c in full]
    print("=" * 90)
    a_b, u_b, l_b = _wf(df, base, cat_base, "BASELINE")
    a_f, u_f, l_f = _wf(df, full, cat_full, "+ calendar")
    print()
    print("=" * 90)
