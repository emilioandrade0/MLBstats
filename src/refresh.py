...
async def refresh_async(
    ...
    # Rebuild derivative parquets (light). merge=True keeps historical rows
    # for events not present in this refresh window — required in production
    # where we ship a built parquet but not the full RAW JSON archive.
    if verbose:
        print("[refresh] rebuild odds_close.parquet (merge)")
    await asyncio.to_thread(build_odds_close, True, not verbose)
    if verbose:
