...
type MultiMeta = { oosSeasons?: number[]; individuals?: Record<string, { n: number; accuracy: number; brier: number }>; consensus?: { accuracy: number; brier: number; bestIndividual: number; improvement: number } | null; consensusUsed?: boolean; note?: string };
type Manifest = { generatedAt: string; source: string; seasons: Season[]; filters: Record<string, { label: string; filters: [string,string][] }>; reports: Record<string, Record<string, unknown>[]>; models: { id: string; label: string; target: string; field: string }[]; multiMeta?: MultiMeta };
...
export default function NflPage() {
  ...
  return <div className="dashboard nfl-dashboard"><a className="skip-link" href="#nfl-content">Saltar al contenido</a><header className="dashboard-header"><SportSwitch sport="NFL"/><a className="dashboard-brand" href="/nfl"><span className="dashboard-brand-icon">🏈</span><span>STRIKE<span>CAST</span><small>NFL / CENTRO DE ANÁLISIS</small></span></a><div className="dashboard-header-right"><span className={`connection-status ${configured?'connected':''}`}><i/>{configured?'Canal NFL configurado':'Canal NFL pendiente'}</span><button className="dashboard-refresh" disabled={loading} onClick={()=>{setLoading(true);setError('');setVersion(v=>v+1);}}><RefreshCw size={15}/>Actualizar datos NFL</button></div></header>
    ...
    {tab==='comparador'&&<><h2>Comparador NFL</h2><p>Cada motor entrena con features distintas y vota su probabilidad de ganador local. El consenso es un promedio simple; solo se persiste si su accuracy OOS supera a todos los motores individuales.</p>{manifest?.multiMeta?.individuals&&<div className="nfl-multi-meta"><span className="eyebrow">Evaluación OOS · {manifest.multiMeta.oosSeasons?.join(' + ')}</span><div>{Object.entries(manifest.multiMeta.individuals).map(([mid,m])=><article key={mid}><span>{manifest.models.find(x=>x.id===mid)?.label||mid}</span><strong>{pct(m.accuracy)}</strong><small>Brier {m.brier.toFixed(3)} · n={m.n}</small></article>)}{manifest.multiMeta.consensus&&<article className={manifest.multiMeta.consensusUsed?'nfl-multi-consensus-used':'nfl-multi-consensus-dropped'}><span>Consenso</span><strong>{pct(manifest.multiMeta.consensus.accuracy)}</strong><small>Δ vs mejor {manifest.multiMeta.consensus.improvement>=0?'+':''}{(manifest.multiMeta.consensus.improvement*100).toFixed(2)} pp · {manifest.multiMeta.consensusUsed?'USADO':'DESCARTADO'}</small></article>}</div></div>}<div className="nfl-table"><table><thead><tr><th>Juego / semana</th>{manifest?.models.filter(m=>m.target==='winner').map(m=><th key={m.id}>{m.label}</th>)}<th>Mercado sin margen · referencia</th><th>L/V/D</th><th>Resultado</th></tr></thead><tbody>{!loading&&displayed.slice(0,limit).map(g=>{const hd=americanToDecimal(g.liveMarket?g.liveMarket.home:g.home_moneyline),ad=americanToDecimal(g.liveMarket?g.liveMarket.away:g.away_moneyline);const p=hd&&ad&&Number.isFinite(hd)&&Number.isFinite(ad)?(1/hd)/(1/hd+1/ad):null;return <tr key={g.game_id}><td>{g.away_team} @ {g.home_team}<small>Semana {g.week} · {when(g.kickoff_utc)}</small></td>{manifest?.models.filter(m=>m.target==='winner').map(m=><td key={m.id}>{modelPick(g,m.field)}</td>)}<td>{p==null?'Sin cuotas':`${p>=.5?g.home_team:g.away_team} · ${pct(Math.max(p,1-p))}`}<small>{g.liveMarket?`Cuota observada: ${when(g.liveMarket.updatedAt)}`:'Cuota del archivo original'}</small></td><td>{g.lvd?.pick?`${g.lvd.pick} · ${pct(g.lvd.confidence)}`:'Sin predicción'}</td><td>{g.status==='final'?`${g.away_score}–${g.home_score}`:'Pendiente'}</td></tr>;})}</tbody></table></div></>}
