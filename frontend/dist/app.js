...
const API_BASE_URL = (window.STRIKECAST_API_BASE_URL || '').replace(/\/$/, '');
...
async function loadDate(d, silent = false) {
  ...
  const response = await fetch(apiUrl(`/api/games?start=${d}&end=${d}&t=${Date.now()}`));
...
async function openDetail(pk, options = {}) {
  ...
  const response = await fetch(apiUrl(`/api/games/${pk}?t=${Date.now()}`));
